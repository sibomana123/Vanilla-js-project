...
const menuItems = document.querySelector('main');
...
const dispalyMenuItems = (menuItem) => {
  let displayMenu = menuItem.map((item) => {
    return ` <div class="item">
        <img src=${item.img} alt="menu_item">
        <div class="info">
          <div class="header">
            <h3>${item.title}</h3>
            <h6>prices: $${item.price}</h6>
          </div>
          <P class="descr">${item.desc}</P>
        </div>
      </div>`;
  });
